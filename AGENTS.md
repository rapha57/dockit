...
- Overlays (icon picker, confirms): house `ModalShell` — one portal, nested lock/Escape stack. No Radix Dialog.
- **Edge fade**: `EdgeFade` host `.edge-fade` (top fade via `::before`, glued to the header with `top: -1px`) + sticky `::after` at the bottom of `[data-edge-scroll]`. Native scrollbar (macOS overlay paints on top of the fade). Linear gradient. Never a mask, never Radix, never an overlay between header and rows. Fade top only after scroll, fade bottom only if more content below. Tables: header stays outside the scroller (`z-index: 2`). No `scrollbar-gutter` on fade panes.
